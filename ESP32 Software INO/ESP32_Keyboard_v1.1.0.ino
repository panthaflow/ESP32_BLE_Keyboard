...
enum UI_STATE { UI_INIT, UI_BLE_WAIT, UI_CONNECTED, UI_TYPING, UI_ACTION, UI_NO_USB };
#endif
...
class MyServerCallbacks: public BLEServerCallbacks {
    void onConnect(BLEServer* pServer) { 
        ...
        if (usb_hid.ready()) updateDisplay(UI_CONNECTED);
    };
    void onDisconnect(BLEServer* pServer) { 
        deviceConnected = false; 
        pServer->startAdvertising(); 
        if (usb_hid.ready()) updateDisplay(UI_BLE_WAIT);
    }
};
...
void setup() {
    ...
    // 1. USB SOFORT INITIALISIEREN (Verhindert den Windows-Timeout)
    ...
    // 2. ERST JETZT DAS DISPLAY STARTEN (Dauert etwas, aber USB läuft schon parallel im Hintergrund)
#if ENABLE_DISPLAY
...
void loop() {
    ...
    if (textBuffer.length() > 0) {
        ...
        if(textBuffer.length() == 0 && deviceConnected) {
             updateDisplay(UI_CONNECTED);
        }
    }
    delay(10);
}