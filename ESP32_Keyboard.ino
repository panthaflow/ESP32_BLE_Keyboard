...
// --- NEU: Puffer für Befehle und Text ---
String textBuffer = "";
...
void typeViaAltCode(char c) {
    ...
    delay(5); // 5ms reichen Windows völlig aus, um den ALT-Status zu erfassen
    ...
    for (int i = 0; i < 4; i++) {
        ...
        delay(4); // 4ms Druckzeit
        
        // Ziffer loslassen (ALT bleibt gedrückt)
        usb_hid.keyboardReport(0, KEYBOARD_MODIFIER_LEFTALT, NULL);
        delay(3); // 3ms Pause zwischen Ziffern
    }

    // 3. ALT loslassen -> Erst JETZT löst Windows den Alt-Code aus
    usb_hid.keyboardRelease(0);
    delay(5); // Kurze Pause zum Verarbeiten vor dem nächsten Zeichen
}
...
void loop() {
    ...
    // WICHTIG: Kurze Pause, damit der ESP32-Watchdog nicht anschlägt
    delay(10);
}